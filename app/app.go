...
import (
	...
	epochsmodule "github.com/evmos/evmos/v16/x/epochs"
	epochsmodulekeeper "github.com/evmos/evmos/v16/x/epochs/keeper"
...
func New(
	...
	fmt.Println(bApp.ChainID())
	keys := newStoreKey(bApp)
	...
	*/
	...
	app.mm = app.newModuleManager(appCodec, encodingConfig, transferModule, icaModule, xonemodule, skipGenesisInvariants, keys)
	...
	// can do so safely.
	genesisModuleOrder := app.newGenesisModuleName()
...
func (app *App) forkGaslimit(ctx sdk.Context) {
	cc, err := app.ConsensusParamsKeeper.Get(ctx)
	if err != nil {
		ctx.Logger().Error("Get consensus params failed", "err", err)
	} else {
...
func (app *App) forkAddEpochs(ctx sdk.Context, forkEpochInfo []epochsmoduletypes.EpochInfo) {
	vm := app.ModuleManager().GetVersionMap()
	// 1. 初始化模块版本号
	vm[epochsmoduletypes.ModuleName] = 1

	// 2. 修复 InitGenesis 参数传递
	epochsGenesis := epochsmoduletypes.NewGenesisState(forkEpochInfo)
	epochsmodule.InitGenesis(ctx, *app.EpochsKeeper, *epochsGenesis)
...
func (app *App) setUpgradeStoreLoader() {
	var addEpochsSkipUpgradeHeight int64
	chainId, err := evmostypes.ParseChainID(app.ChainID())
	if err != nil {
		app.Logger().Error("Get consensus params failed", "err", err)
		return
	} else {
		if chainId.Int64() == evmostypes.EvmChainID_Mainnet {
			addEpochsSkipUpgradeHeight = ForkAddEpochsMainNetHeight
		} else if chainId.Int64() == evmostypes.EvmChainID_Testnet {
			addEpochsSkipUpgradeHeight = ForkAddEpochsTestNetHeight
		} else {
			return
		}

		app.SetStoreLoader(upgradetypes.UpgradeStoreLoader(
			addEpochsSkipUpgradeHeight,
...
func (app *App) Commit() abci.ResponseCommit {
	...
	if err != nil {
		...
		if chainId.Int64() == evmostypes.EvmChainID_Mainnet {
			if height == ForkAddEpochsMainNetHeight-1 {
				app.Logger().Info("Please restart the node to load the epochs module and the store of the epochs module")
				os.Exit(0)
			}

		} else if chainId.Int64() == evmostypes.EvmChainID_Testnet {
			if height == ForkAddEpochsTestNetHeight-1 {
				app.Logger().Info("Please restart the node to load the epochs module and the store of the epochs module")
				os.Exit(0)
			}
		}
...
func newStoreKey(bapp *baseapp.BaseApp) map[string]*storetypes.KVStoreKey {
	baseKeys := []string{
	...
	addEpochsKey := func(height int64, forkHeight int64) bool {
		return height >= forkHeight
	}

	height := bapp.LastBlockHeight() + 1
	chainId, err := evmostypes.ParseChainID(bapp.ChainID())
	if err != nil {
		bapp.Logger().Error("Get consensus params failed", "err", err)
		panic(fmt.Errorf("failed to parse chain id: %s", err))
	}

	switch chainId.Int64() {
	case evmostypes.EvmChainID_Mainnet:
		if addEpochsKey(height, ForkAddEpochsMainNetHeight) {
			baseKeys = append(baseKeys, epochsmoduletypes.StoreKey)
		}
	case evmostypes.EvmChainID_Testnet:
		if addEpochsKey(height, ForkAddEpochsTestNetHeight) {
			baseKeys = append(baseKeys, epochsmoduletypes.StoreKey)
		}
	default:
		bapp.Logger().Info("Customize chainid and do not execute fork", "chainId", chainId.Int64())
	}

	return sdk.NewKVStoreKeys(baseKeys...)
}

func (app *App) newModuleName() []string {
	moduleName := []string{
	...
	case evmostypes.EvmChainID_Mainnet:
		if addEpochsKey(height, ForkAddEpochsMainNetHeight) {
			moduleName = append(moduleName, epochsmoduletypes.ModuleName)
		}
	case evmostypes.EvmChainID_Testnet:
		if addEpochsKey(height, ForkAddEpochsTestNetHeight) {
			moduleName = append(moduleName, epochsmoduletypes.ModuleName)
	...
	default:
		app.Logger().Info("Customize chainid and do not execute fork", "chainId", chainId.Int64())
	}

	return moduleName
}

func (app *App) newGenesisModuleName() []string {
	genesisModuleName := []string{
		capabilitytypes.ModuleName,
		authtypes.ModuleName,
		banktypes.ModuleName,
		distrtypes.ModuleName,
	...
	case evmostypes.EvmChainID_Mainnet:
		if addEpochsKey(height, ForkAddEpochsMainNetHeight) {
			genesisModuleName = append(genesisModuleName, epochsmoduletypes.ModuleName)
		}
	case evmostypes.EvmChainID_Testnet:
		if addEpochsKey(height, ForkAddEpochsTestNetHeight) {
			genesisModuleName = append(genesisModuleName, epochsmoduletypes.ModuleName)
	...
	return genesisModuleName
}

func (app *App) newModuleManager(appCodec codec.Codec, encodingConfig simappparams.EncodingConfig, transferModule transfer.AppModule,
	icaModule ica.AppModule, xonemodule xonemodule.AppModule, skipGenesisInvariants bool, keys map[string]*storetypes.KVStoreKey) *module.Manager {
	// 基础模块列表
	...
	// 动态添加模块逻辑
	addEpochsModule := func(height int64, forkHeight int64) bool {
		return height >= forkHeight
	}
	...
	switch chainId.Int64() {
	case evmostypes.EvmChainID_Mainnet:
		if addEpochsModule(height, ForkAddEpochsMainNetHeight) {
			app.EpochsKeeper = epochsmodulekeeper.NewKeeper(
				appCodec,
				keys[epochsmoduletypes.StoreKey],
			)

			app.EpochsKeeper.SetHooks(epochsmodulekeeper.MultiEpochHooks{})

			epochsModule := epochsmodule.NewAppModule(appCodec, *app.EpochsKeeper)
			baseModules = append(baseModules, epochsModule)
		}
	case evmostypes.EvmChainID_Testnet:
		if addEpochsModule(height, ForkAddEpochsTestNetHeight) {
			app.EpochsKeeper = epochsmodulekeeper.NewKeeper(
				appCodec,
				keys[epochsmoduletypes.StoreKey],
			)
	...
	// 返回模块管理器
	return module.NewManager(baseModules...)
}
